...
class ComplexInputNetwork(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, 
        ...
        config = {"conv_filters": model_config.get("conv_filters", get_filter_config((84, 84, 3))),
...
class Vanilla_PPO(TorchModelV2, nn.Module):
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        #format obs
        input_dict = torch.cat((input_dict["obs"]["command"], 
                                input_dict["obs"]["d2target"],
                                input_dict["obs"]["pitch"],
                                input_dict["obs"]["roll"],
                                input_dict["obs"]["velocity_mag"],
                                input_dict["obs"]["yaw"]), 
                                1)
