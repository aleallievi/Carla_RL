...
class CarlaCore(object):
    ...
    def step(self, timeout, action=None):
        ...
        # state = [self.rgb_cam]
        state = [self.process_img(img, 84, 84) for img in state]
        state = [state, np.float32(velocity_mag), np.float32(d2target), np.float32(rotation.pitch), np.float32(rotation.yaw), np.float32(rotation.roll)]
        ...
        return state, reward, done, {"score_composed":self.statistics_manager.route_record["score_composed"],
                                     "route_percentage":self.statistics_manager.route_record['route_percentage'], 
                                     "n_collisions:":self.infractions_test.n_collisions,
                                     "n_tafficlight_violations": self.infractions_test.n_tafficlight_violations, 
                                     "n_stopsign_violations":self.infractions_test.n_stopsign_violations,
                                     "n_route_violations": self.infractions_test.n_route_violations,
                                     "n_vehicle_blocked":self.infractions_test.n_vehicle_blocked}
